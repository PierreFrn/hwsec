...
class trContext:
    ...
    def __init__ (self, filename, max):
        ...
        with open(str(filename), 'rb') as f:
            ...
            magic = bytes.decode(magic)
            if magic != HWSECMAGICNUMBER:
                raise ValueError ('wrong magic number; is this a real HWSec trace file?' % magic)
...
def plot (prefix, i, t):
    ...
    with open ("%s.cmd" % str(prefix), 'w') as fpc:
        ...
        with open (fname, 'w') as fpd:
            fpc.write ("set terminal pngcairo size 1280,720 enhanced font 'Verdana,10'\n")
            fpc.write ("set output '%s.png'\n" % str(prefix))
            fpc.write ("plot \\\n")
